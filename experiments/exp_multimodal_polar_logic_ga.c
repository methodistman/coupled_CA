/*
 * exp_multimodal_polar_logic_ga: Multimodal logic using polar coordinate
 * encoding and diagonal slicing.  Each cell is addressed by (r, theta)
 * relative to the north-edge center.  The grid is treated as a bundle of
 * 1D "traces" (rows, columns, diagonals, anti-diagonals).  Each mode
 * activates a different pair of traces for input/output.
 *
 * Modes:
...
/* ===== Polar coordinate system: north-center origin ===== */
static inline double polar_r(int size, int x, int y) {
    double dx = x - size / 2.0;
    double dy = y; /* north edge is y=0 */
    return sqrt(dx * dx + dy * dy);
}

static inline double polar_theta(int size, int x, int y) {
    double dx = x - size / 2.0;
    double dy = y;
    return atan2(dy, dx); /* -PI to PI, 0=east */
}
...
static void pop_randomize(Population *p, uint64_t (*rng)(void)) {
    for (int i = 0; i < p->pop_size; i++)
        for (int j = 0; j < p->ncells; j++) {
            int weight = (int)(rng() % 16);
            int orient = (int)(rng() % 8);
            p->pop[i].genomes[j] = genome_pack_oriented(0, weight, 0, orient);
        }
}
...
int main(int argc, char **argv) {
    ...
    int mutate_mask = GENOME_MUTATE_WEIGHT | GENOME_MUTATE_ORIENTATION;
    ...
    sys.pipeline = pipeline_preset_genomic_orientation(INTENT_MODE_REPLACE, 0.0f,
                                                        0, -1, 0, 0, 1.0,
                                                        GENOME_MUTATE_ALL);
    ...
    printf("# exp_multimodal_polar_logic_ga: size=%d pop=%d gens=%d steps=%d seed=%llu\n",
