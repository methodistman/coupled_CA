/*
 * exp_multimodal_wave_logic_ga: Multimodal logic on the wave substrate.
 *
 * Uses a single wave grid with per-cell genomes.  Two evaluation modes:
 *   Mode 0 (horizontal): input A/B -> left edge as wave values,
 *                        output read from right edge wave values
 *   Mode 1 (vertical):   input A/B -> top edge as wave values,
 *                        output read from bottom edge wave values
 *
 * The wave substrate gives us:
 *   - Wave values 0..255 (gradient state, not just binary)
 *   - coords.x / coords.y per cell (positional encoding)
 *   - Vector rules (vector_and, vector_or, vector_xor) that respect
 *     directional wave propagation
 *
 * This tests whether the richer state space enables two independent
 * computational modes on one substrate — a form of spatial
 * programmability.
 */
#include <stdio.h>
#include <stdlib.h>
...
/* --- input injection on wave grid edges --- */
static void set_input_left(WaveGrid *g, int a, int b) {
    int sz = g->size, half = sz / 2;
    for (int y = 0; y < sz; y++) {
        uint8_t val = (y < half) ? (a ? WAVE_HIGH : WAVE_LOW) : (b ? WAVE_HIGH : WAVE_LOW);
        g->cells[y * sz + 0].wave = val;
        g->cells[y * sz + 0].alive = (val > 0);
    }
}

static void set_input_top(WaveGrid *g, int a, int b) {
    int sz = g->size, half = sz / 2;
    for (int x = 0; x < sz; x++) {
        uint8_t val = (x < half) ? (a ? WAVE_HIGH : WAVE_LOW) : (b ? WAVE_HIGH : WAVE_LOW);
        g->cells[0 * sz + x].wave = val;
        g->cells[0 * sz + x].alive = (val > 0);
    }
}

/* --- output readout: compare edge wave sum to grid mean --- */
static int read_output_right(const WaveGrid *g) {
    int sz = g->size, half = sz / 2;
    uint64_t edge_sum = 0;
    uint64_t grid_sum = 0;
    for (int y = 0; y < half; y++) edge_sum += g->cells[y * sz + (sz - 1)].wave;
    for (int i = 0; i < sz * sz; i++) grid_sum += g->cells[i].wave;
    uint64_t expected_edge = grid_sum * half / (sz * sz);
    return (edge_sum > expected_edge) ? 1 : 0;
}

static int read_output_bottom(const WaveGrid *g) {
    int sz = g->size, half = sz / 2;
    uint64_t edge_sum = 0;
    uint64_t grid_sum = 0;
    for (int x = 0; x < half; x++) edge_sum += g->cells[(sz - 1) * sz + x].wave;
    for (int i = 0; i < sz * sz; i++) grid_sum += g->cells[i].wave;
    uint64_t expected_edge = grid_sum * half / (sz * sz);
    return (edge_sum > expected_edge) ? 1 : 0;
}
...
/* --- evaluation --- */
static double evaluate_mode(WaveSystem *sys, const int *expected,
                            int eval_steps,
                            void (*set_in)(WaveGrid*,int,int),
                            int (*read_out)(const WaveGrid*),
                            int *out_truth) {
    int sz = sys->grids[0]->size;
    int n = sz * sz;
    WaveCell *bk = malloc(n * sizeof(WaveCell));
    memcpy(bk, sys->grids[0]->cells, n * sizeof(WaveCell));
    ...
    for (int a = 0; a <= 1; a++) {
        for (int b = 0; b <= 1; b++) {
            memcpy(sys->grids[0]->cells, bk, n * sizeof(WaveCell));
            for (int st = 0; st < eval_steps; st++) {
                wave_sys_step_genomic(sys);
...
static double evaluate_individual(WaveSystem *sys, const int *exp0, const int *exp1,
                                  int eval_steps, int fit_mode,
                                  const WaveCell *bk_cells,
                                  int *truth0, int *truth1) {
    int n = sys->grids[0]->size * sys->grids[0]->size;
    memcpy(sys->grids[0]->cells, bk_cells, n * sizeof(WaveCell));
...
static void pop_evaluate(Population *p, WaveSystem *sys,
                         const int *exp0, const int *exp1,
                         int eval_steps, int fit_mode,
                         const WaveCell *bk_cells) {
    for (int i = 0; i < p->pop_size; i++) {
        for (int j = 0; j < p->ncells; j++)
            sys->grids[0]->cells[j].genome = p->pop[i].genomes[j];
...
int main(int argc, char **argv) {
    ...
    rng_seed(seed);
    WaveSystem sys;
    wave_sys_init(&sys, 1, size);
    sys.grids[0]->active = 1;
    sys.grids[0]->rule_idx = 0; /* wave_rule_life default */
    wave_sys_randomize(&sys, rng_u64);

    /* Give the grid some genomes */
    for (int i = 0; i < size * size; i++)
        sys.grids[0]->cells[i].genome = genome_random(rng_u64);

    Population *popu = pop_create(size, pop);
    pop_randomize(popu, rng_u64);

    int n = size * size;
    WaveCell *bk_cells = malloc(n * sizeof(WaveCell));
    memcpy(bk_cells, sys.grids[0]->cells, n * sizeof(WaveCell));

    printf("# exp_multimodal_wave_logic_ga: gate0=%s gate1=%s size=%d pop=%d gens=%d steps=%d seed=%llu\n",
    ...
    if (save_genomes) {
        ...
        if (fp) {
            ...
            fwrite(popu->pop[best].genomes, sizeof(CellGenome), n, fp);
            fclose(fp);
