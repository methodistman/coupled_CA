/*
 * exp_wave_logic_ga: Evolve per-cell genomes on a wave grid to compute
...
#include "../ca_core/wave_coupling.h"
...
static void set_input(WaveGrid *g, int a, int b) {
    int sz = g->size, half = sz / 2;
    for (int y = 0; y < sz; y++) {
        for (int x = 0; x < half; x++) {
            WaveCell c = wave_grid_get(g, x, y);
            c.alive = (uint8_t)a;
            wave_grid_set(g, x, y, c);
        }
...
static int read_output(const WaveGrid *g) {
    int sz = g->size, half = sz / 2;
    int c = 0;
    for (int y = 0; y < half; y++) {
        WaveCell cell = wave_grid_get(g, sz - 1, y);
        c += cell.alive;
    }
    int total = 0, n = sz * sz;
...
static double evaluate(WaveSystem *sys, const int *expected,
                       int eval_steps,
...
int main(int argc, char **argv) {
    ...
    WaveSystem sys;
    wave_sys_init(&sys, 2, size);
    sys.grids[0]->active = 1;
    sys.grids[0]->rule_idx = 0;
    sys.grids[1]->active = 1;
    sys.grids[1]->rule_idx = 0;

    wave_coupling_connect(&sys.coupling, 0, EDGE_LEFT, 1, EDGE_RIGHT, 1.0f);
    ...
    printf("# exp_wave_logic_ga: evolve wave grid genomes for logic gate\n");
