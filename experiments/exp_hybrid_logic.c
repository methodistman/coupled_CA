/*
 * exp_hybrid_logic: Evolve per-cell genomes on a payload grid that is
 * modulated by a binary control grid, to compute a target logic gate.
 *
 * Setup: binary grid 0 = input buffer (left=A, right=B).
 *        payload grid 0 = computation grid with genomes.
 *        Cross-connect: binary B0:right -> payload P0:left.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void usage(const char *prog) {
    ...
    fprintf(stderr, "  --steps N                Evaluation steps (default %d)\n", DEF_STEPS);
...
static void set_binary_input(Grid *g, int a, int b) {
...
static int read_payload_output(const PayloadGrid *g) {
    int sz = g->size, half = sz / 2;
    int c = 0;
...
static double evaluate(HybridSystem *sys, const int *expected,
                       int eval_steps,
                       const uint8_t *bb, const uint8_t *bp,
                       int *out_truth) {
    int sz = sys->binary.grids[0]->size;
    int ncells = sz * sz;
    ...
    for (int a = 0; a <= 1; a++) {
        for (int b = 0; b <= 1; b++) {
            memcpy(sys->binary.grids[0]->cells, bb, ncells);
            for (int i = 0; i < ncells; i++) sys->payload.grids[0]->cells[i].alive = bp[i];
            for (int st = 0; st < eval_steps; st++) {
                hybrid_sys_step(sys);
                set_binary_input(sys->binary.grids[0], a, b);
            }
            int o = read_payload_output(sys->payload.grids[0]);
            int idx = (a << 1) | b;
            truth |= (o << idx);
            if (o == expected[idx])
...
static void pop_evaluate(Population *p, HybridSystem *sys,
                         const int *expected, int eval_steps,
                         const uint8_t *bb, const uint8_t *bp) {
    for (int i = 0; i < p->pop_size; i++) {
        memcpy(sys->payload.grids[0]->genomes, p->pop[i].genomes,
...
int main(int argc, char **argv) {
    ...
    sys.binary.grids[0]->active = 1;
    sys.binary.grids[0]->rule_idx = 0;
    sys.payload.grids[0]->active = 1;
    sys.payload.grids[0]->rule_idx = 0;

    /* Cross-connect binary input -> payload computation */
    hybrid_xconn_add(&sys, HYBRID_BINARY, 0, EDGE_RIGHT,
                     HYBRID_PAYLOAD, 0, EDGE_LEFT);

    payload_grid_alloc_genomes(sys.payload.grids[0]);
    hybrid_sys_randomize(&sys, rng_u64);

    /* Payload pipeline with genomic stepping, no local GA */
    sys.payload.pipeline = payload_pipeline_preset_genomic(
        INTENT_MODE_REPLACE, 0.0f, 0, -1, 0, 0, 1.0, GENOME_MUTATE_ALL);
    ...
    uint8_t *bb_main = malloc(ncells_main);
    uint8_t *bp_main = malloc(ncells_main);
    if (!bb_main || !bp_main) { fprintf(stderr, "OOM\n"); return 1; }
    memcpy(bb_main, sys.binary.grids[0]->cells, ncells_main);
    for (int i = 0; i < ncells_main; i++) bp_main[i] = sys.payload.grids[0]->cells[i].alive;

    printf("# exp_hybrid_logic: evolve payload genomes with binary control for logic gate\n");
    ...
    for (int g = 0; g < gens; g++) {
        pop_evaluate(popu, &sys, expected, steps, bb_main, bp_main);
